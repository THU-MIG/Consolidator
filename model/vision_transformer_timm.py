...
default_cfgs = {
    ...
    'vit_base_patch16_224_mocov3': _cfg(url=''),
}
...
class Adapter(nn.Module):
    def __init__(self,
        ...
        print('adapter', self.reduction_dims)

        self.identity = False
        if self.reduction_dims == 0 or self.embed_dims == 0:
            self.identity = True

        if self.reduction_dims > 0:
            self.ln1 = nn.Linear(self.embed_dims, self.reduction_dims)
            self.activate = QuickGELU()
            self.ln2 = nn.Linear(self.reduction_dims, self.embed_dims)
            self.init_weights()
...
class Consolidator(nn.Module):
    def __init__(self, in_features, out_features, rep_drop=0., with_channel_shuffle=False, fc_groups=(384,), drop_type='droppath', rep_bias=True, bias=True):
        super().__init__()
        self.out_features = out_features
        self.in_features = in_features
        self.fc_origin = nn.Conv1d(in_features, out_features, 1, bias=bias)
        self.fc_rep_branches = nn.ModuleList()
        if rep_drop == 0:
            self.drop_layer = nn.Identity()
            self.union_drop_layer = nn.Identity()
        elif drop_type == 'droppath':
            self.drop_layer = DropPath(rep_drop)
            self.union_drop_layer = nn.Identity()
        else:
            raise NotImplementedError

        if len(fc_groups) > 1 or fc_groups[0] > 0:
            for g in fc_groups:
                new_branch = nn.Sequential(OrderedDict([
                    ('channel_shuffle', ChannelShuffle1d(g) if with_channel_shuffle else nn.Identity()),
                    ('group_fc', nn.Conv1d(in_features, out_features, 1, groups=g, bias=rep_bias)),
                    ('drop', self.drop_layer)])
                )

                new_branch.group_fc.weight.data.uniform_(-REP_BRANCH_INIT_STD, REP_BRANCH_INIT_STD)
                if new_branch.group_fc.bias is not None:
                    new_branch.group_fc.bias.data.uniform_(-REP_BRANCH_INIT_STD, REP_BRANCH_INIT_STD)
                self.fc_rep_branches.append(new_branch)


    def channel_shuffle_for_matrix(self, matrix, groups):
        c_out, c_in = self.out_features, self.in_features
        channels_per_group = c_in // groups
        matrix = matrix.view(c_out, groups, channels_per_group)
        matrix = torch.transpose(matrix, 1, 2).contiguous()
        matrix = matrix.view(c_out, c_in)
        return matrix
    ...
    def actual_params(self):
        ...
        tuned_params = 0
        for b in self.fc_rep_branches:
            g = b.group_fc.groups
            tmp_mask = torch.zeros((self.out_features, self.in_features))
            channels_per_out_group = self.out_features // g
            channels_per_in_group = self.in_features // g
            for g_i in range(g):
                tmp_mask[(g_i*channels_per_out_group):((g_i+1)*channels_per_out_group), (g_i*channels_per_in_group):((g_i+1)*channels_per_in_group)] += 1
            if isinstance(b.channel_shuffle, ChannelShuffle1d):
                tmp_mask = self.channel_shuffle_for_matrix(tmp_mask, g)
            mask += tmp_mask

            tuned_params += self.out_features * self.in_features / b.group_fc.groups
        saved_params = (mask > 0).sum().item()
        return saved_params, tuned_params-saved_params

    def get_actual_params(self):
        weight = self.fc_origin.weight.data.cpu().detach().clone().squeeze()
        if self.fc_origin.bias is not None:
            bias = self.fc_origin.bias.data.cpu().detach().clone()
        else:
        ...
        for b in self.fc_rep_branches:
            ...
            channels_per_in_group = self.in_features // g
            for g_i in range(g):
                tmp_mask[(g_i*channels_per_out_group):((g_i+1)*channels_per_out_group), (g_i*channels_per_in_group):((g_i+1)*channels_per_in_group)] += b.group_fc.weight[(g_i*channels_per_out_group):((g_i+1)*channels_per_out_group)].cpu().squeeze(dim=2)
            if isinstance(b.channel_shuffle, ChannelShuffle1d):
                tmp_mask = self.channel_shuffle_for_matrix(tmp_mask, self.in_features // g)
            weight += tmp_mask
            if b.group_fc.bias is not None:
                bias += b.group_fc.bias

        return weight, bias

    def rand_init(self, STD=1e-1):
        self.fc_origin.weight.data.uniform_(-STD, STD)
        if self.fc_origin.bias is not None:
            self.fc_origin.bias.data.uniform_(-STD, STD)
        for b in self.fc_rep_branches:
            b.group_fc.weight.data.uniform_(-STD, STD)
            if b.group_fc.bias is not None:
                b.group_fc.bias.data.uniform_(-STD, STD)


    def forward(self, x):
        # x: [B, N, C]
        x = x.transpose(1, 2)  # [B, C, N]
        out = self.fc_origin(x)
        out_rep = None
        for b in self.fc_rep_branches:
            if out_rep is None:
                out_rep = b(x)
        ...
        if out_rep is not None:
            out += self.union_drop_layer(out_rep)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,LoRA_dim=1024, prefix_dim=1024, drop_rate_LoRA=0,
                 rep_drop=0., with_channel_shuffle=False, fc_groups=(384,), drop_type='droppath', rep_bias=True):
        super().__init__()
        ...
        if fc_groups[0] <= 0:
            ...
            self.qkv = Consolidator(dim, dim * 3, bias=qkv_bias,
                             rep_drop=rep_drop, with_channel_shuffle=with_channel_shuffle, fc_groups=fc_groups, drop_type=drop_type,
                             rep_bias=rep_bias)
        ...
        if fc_groups[0] <= 0:
            ...
            self.proj = Consolidator(dim, dim,
                              rep_drop=rep_drop, with_channel_shuffle=with_channel_shuffle, fc_groups=fc_groups,
                              drop_type=drop_type,
                              rep_bias=rep_bias)
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
        ...
        if fc_groups[0] <= 0:
            ...
            self.fc1 = Consolidator(in_features, hidden_features,
                             rep_drop=rep_drop, with_channel_shuffle=with_channel_shuffle, fc_groups=fc_groups, drop_type=drop_type,
                             rep_bias=rep_bias)
        self.act = act_layer()
        ...
        if fc_groups[0] <= 0:
            ...
            self.fc2 = Consolidator(hidden_features, out_features,
                         rep_drop=rep_drop, with_channel_shuffle=with_channel_shuffle, fc_groups=fc_groups, drop_type=drop_type,
                         rep_bias=rep_bias)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',prompt_tuning_dim=1024,LoRA_dim=1024,adapter_dim=1024,
                 prefix_dim=1024,drop_rate_LoRA=0,drop_rate_prompt=0,drop_rate_adapter=0, IS_not_position_VPT=False,
                 rep_drop=0., with_channel_shuffle=False, fc_groups=(0,), drop_type='droppath', rep_bias=True):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                rep_drop=rep_drop, with_channel_shuffle=with_channel_shuffle, fc_groups=fc_groups,
                drop_type=drop_type, rep_bias=rep_bias, last_prompt_dim=0 if i == 0 else prompt_tuning_dim
            )
        ...
        self.consolidator_dim = None if fc_groups[0] <= 0 else fc_groups
    ...
    def freeze_stages(self):
        ...
        for block in self.blocks:
            ...
            if self.consolidator_dim is not None:
                for n, m in block.named_modules():
                    if 'rep' in n:
                        print('setting train:', n)
                        m.train()

            
        for name,param in self.named_parameters():
            if 'adapter' not in name and 'prompt' not in name and 'LoRA' not in name and 'prefix' not in name and 'head' not in name and 'rep' not in name:
                print('freezing', name)
                param.requires_grad = False

        for name,param in self.named_parameters():
            if 'norm' in name:
                print('unfreezing extra:', name)
                param.requires_grad = True

        if self.consolidator_dim is not None:
            for name,param in self.named_parameters():
                if 'block' in name and 'bias' in name and 'norm' not in name:
                    print('unfreezing extra:', name)
                    param.requires_grad = True
        ...
        for name,param in self.named_parameters():
            ...
            if param.requires_grad:
                ...
                if 'adapter' in name:
                    ...
                    consolidator_para_nums += param.numel()
                
                
        print('parameters:',total_para_nums,'adapter',adapter_para_nums,'LoRA',LoRA_para_nums,'prompt',vp_para_nums,'consolidator',consolidator_para_nums,'head',head_para_nums)
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        ...
        print('parameters:',total_para_nums,'adapter',adapter_para_nums,'LoRA',LoRA_para_nums,'prompt',vp_para_nums,'prefix',prefix_para_nums,'consolidator',consolidator_para_nums,'head',head_para_nums,'model',model_para_nums)
    ...
    def forward_features(self, x):
        ...
        if self.prompt_tuning_dim != 0:
            visual_prompt_tokens = self.visual_prompt_token[:,:self.prompt_tuning_dim,:].expand(B,-1,-1)
            if not self.IS_not_position_VPT:
                visual_prompt_tokens = visual_prompt_tokens + self.visual_prompt_token_pos_embed[:,:self.prompt_tuning_dim,:]
            visual_prompt_tokens = self.drop_prompt(visual_prompt_tokens)
            x = torch.cat((x,visual_prompt_tokens), dim=1)
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    """
    if 'ViT-B_16.npz' in checkpoint_path:
        import numpy as np
        print('loading from 21k supervised ckpt {}...'.format(checkpoint_path))
        def _n2p(w, t=True):
    ...
    elif 'vit-b-300ep.pth.tar' in checkpoint_path:
        # mocov3
        print('loading from mocov3 ckpt {}...'.format(checkpoint_path))
        _loading_weights_mocov3(model, checkpoint_path)
    else:
        raise NotImplementedError
...
def _loading_weights_mocov3 (model, mocov3_base_ckpt = 'ckpts/vit-b-300ep.pth.tar'):
    #mocov3_base_ckpt = 'ckpts/vit-b-300ep.pth.tar'
    assert os.path.isfile(mocov3_base_ckpt)
    print("=> loading checkpoint '{}'".format(mocov3_base_ckpt))
    checkpoint = torch.load(mocov3_base_ckpt, map_location="cpu")
    ...
    for key in list(state_dict.keys()):
        if 'mlp.fc1' in key or 'mlp.fc2' in key or 'attn.qkv' in key or 'attn.proj' in key:
            if key.endswith('weight'):
                state_dict.update(
                    {key[:-len('weight')] + 'fc_origin.weight': state_dict[key].unsqueeze(-1)})
            elif key.endswith('bias'):
                state_dict.update({key[:-len('bias')] + 'fc_origin.bias': state_dict[key]})
            else:
                raise NotImplementedError
    msg = model.load_state_dict(state_dict, strict=False)
    # assert set(msg.missing_keys) == {"head.weight", "head.bias"}
    print(msg)
    print("=> loaded pre-trained model '{}'".format(mocov3_base_ckpt))
    return model
...
def vit_base_patch16_224_mocov3(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_mocov3', pretrained=pretrained, **model_kwargs)
    return model